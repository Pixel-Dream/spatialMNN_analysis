...
def mender_mem_wrapper(file_list, scale, mode, radius, nSeed, batch, msm_res):
    start_time = time.time()
    mem_usage, adata_out = memory_usage((mender_run, (file_list, scale, mode, radius, nSeed, batch, msm_res)), retval=True, max_usage=True)
    end_time = time.time()

    print(f"Maximum memory usage: {mem_usage} MiB")
    print(f"Total running time: {end_time - start_time} seconds")
    rtn_list = []
    rtn_list.append(adata_out)
    rtn_list.append({"mem":mem_usage, 
                       "tic":start_time,
                       "toc":end_time})
    return rtn_list
