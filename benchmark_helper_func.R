...
# Record results
rkd_res <- function(bench_df, tic, toc, mem_usage=NULL, method_name,
                    seu_ls=NULL, label_col=NULL, result_col=NULL,
                    label_vec=NULL, result_vec=NULL, sample_vec = NULL, mem=NULL){
  idx <- which(bench_df$method == method_name)
  bench_df$tic[idx] <- tic
  bench_df$toc[idx] <- toc
  bench_df$time[idx] <- toc - tic
  if(!is.null(mem))bench_df$mem[idx] <- mem #for python
  else if(!is.null(mem_usage))bench_df$mem[idx] <- mem_usage$Peak_RAM_Used_MiB # peakRAM 
