...
R_SCRIPT="/users/hzhou1/benchmark/fig2_LIBD/benchmark.R"
# Define the base output path
OUTPUT_BASE_PATH="/users/hzhou1/benchmark/fig2_LIBD/result"
...
for OUTPUT_PATH in "${OUTPUT_PATHS[@]}"; do
    ...
    for ARGS in "${R44[@]}"; do
        submit_job "$ARGS" "80G" "conda_R/4.4" "$OUTPUT_PATH"
    done
    
    # MENDER
    submit_job "5 MENDER" "64G" "R/4.3" "$OUTPUT_PATH"
    # SLAT
    for (( i=1; i<=NUM_SLAT_RUNS; i++ )); do
        ARGS=("$((i+5)) SLAT")
        submit_job "$ARGS" "64G" "R/4.3" "$OUTPUT_PATH" "$i"
    done
