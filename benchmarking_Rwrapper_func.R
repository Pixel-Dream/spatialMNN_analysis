...
create_annFiles <- function(spe, annots_label, sample_label,
  ...
  #detach("package:Matrix", unload = T)
  ...
  for (i in samples) {
    ...
    metaData = data.frame(domainAnnotations = as.character(colData(speX)[, annots_label]),
    ...
    write_h5ad(ad, file.path(fileSavepath, paste0(sample, ".h5ad")))
...
runSLAT <- function(python_path, pyscript_path,
  ...
  #   borrowing.
  # ref_sample reference sample No.

  use_python(python_path)
  print(paste("Python environment set up.", Sys.time()))
  source_python(file.path(pyscript_path, "benchmark_slatFunc.py"))
  print(paste("Python script sourced.", Sys.time()))
  # Performing slat multislice runs, where every sample is
  # used as reference once.
  ad_out = NULL
  message(paste("Ref sample index:", ref_sample),"Total:",length(samplePaths_vec))
  stopifnot("Reference sample index exceeded" = ref_sample <= length(samplePaths_vec))
  ...
  filenum = ref_sample
    ...
    tmp_ad = slat_mem_wrapper(ad_list, cos)
    names(tmp_ad) = c(ad_names,"stats")
    tmp_ad[["stats"]]$avg_ari <- sapply(ad_names,
                                    function(x){
                                        ARI(tmp_ad[[x]]$obs[[domains]],
                                            tmp_ad[[x]]$obs$clust_ref)
                                    }) %>% mean

    #if(is.null(ad_out)){
    #    ad_out = tmp_ad
    #}else{
    #    if(tmp_ad[["stats"]]$avg_ari > ad_out[["stats"]]$avg_ari){
    #        ad_out = tmp_ad
    #    }
    #}
...
runMENDER <- function(python_path, pyscript_path,
  ...
  use_python(python_path)
  print(paste("Python environment set up.", Sys.time()))
  source_python(file.path(pyscript_path, "benchmark_menderFunc.py"))
  print(paste("Python script sourced.", Sys.time()))
  ...
  mender_out = mender_mem_wrapper(samplePaths_vec, scale, mode, radius,
                          seed, batch, msm_res)
